		...
		*/
		...
		if(strncmp(buffer, "send ", 5) == 0) {
			...
			*tmp = '\0';
			...
			sprintf(formatted_msg, "[%04d/%02d/%02d] (%02d:%02d:%02d) %s: %s",1900 + now->tm_year, now->tm_mon + 1, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec, cnode->username, msg);
			/* logging in the server */
			printf("{%04d/%02d/%02d] (%02d:%02d:%02d) %s sent msg to %s\n",1900 + now->tm_year, now->tm_mon + 1, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec, cnode->username, targetnode->username);
